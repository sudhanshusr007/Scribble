...
const AddEditNotes = ({ noteData, type, getAllNotes, onClose, showToastMessage }) => {
  ...
  const addNewNote = async () => {
    try {
      const response = await axiosInstance.post('https://scribble-server-z5ir.onrender.com/add-note', {
        title,
        content,
        tags,
      });
  ...
  const editNote = async () => {
    ...
    try {
      const response = await axiosInstance.put('https://scribble-server-z5ir.onrender.com/' + noteId, {
        title,
        content,
        tags,
      });
